...
public class OpswiseModelManager {
    ...
    public boolean doesTaskExists(String task_name) {
        if(dbc.executeQuery(String.format(SYSID_FOR_TASK,task_name)) == null)
            return false;
        else
            return true;
    }
    ...
    public boolean doesTriggerExists(String trigger_name) {
        if (dbc.executeQuery(String.format(SYSID_FOR_TRIGGER,trigger_name)) == null)
            return false;
        else
            return true;
    ...
    public boolean isTriggerEnabled(String trigger_name) {
        if (dbc.executeQuery(String.format(IS_TASK_ENABLED,trigger_name)) == null)
            return false;
        else
            return true;
    }
